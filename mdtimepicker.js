...
+function ($) {
	...
	var Time = function (hour, minute) {
		...
		this.invert = function () {
			if (this.getT() === 'ق.ظ') this.setHour(this.getHour() + 12);
		...
		this.getT = function() { return this.hour < 12 ? 'ق.ظ' : 'ب.ظ'; };
	};
	...
	var MDTimePicker = function (input, config) {
		...
		this.timepicker = {
			...
			timeHolder : {
				...
				am_pm: $('<span class="mdtp__ampm">ق.ظ</span>')
			},
			clockHolder : {
				wrapper: $('<section class="mdtp__clock_holder"></section>'),
				am: $('<span class="mdtp__am">ق.ظ</span>'),
				pm: $('<span class="mdtp__pm">ب.ظ</span>'),
				clock: {
				...
				buttonsHolder : {
					...
					btnOk : $('<span class="mdtp__button ok">تائید</span>'),
					btnCancel: $('<span class="mdtp__button cancel">انصراف</span>')
				}
			}
		};
		...
		picker.clockHolder.am.click(function () { if (that.selected.getT() !== 'ق.ظ') that.setT('am'); });
		picker.clockHolder.pm.click(function () { if (that.selected.getT() !== 'ب.ظ') that.setT('pm'); });
	...
	MDTimePicker.prototype = {
			...
			for (var i = 0; i < 12; i++) {
				...
				hour.find('span').click(function () {
					var _data = parseInt($(this).parent().data('hour')),
						...
						_value = (_data + ((_selectedT === 'ب.ظ' && _data < 12) || (_selectedT === 'ق.ظ' && _data === 12) ? 12 : 0)) % 24;
		...
		setT : function (value) {
			...
			this.timepicker.clockHolder.am[t === 'ق.ظ' ? 'addClass' : 'removeClass']('active');
			this.timepicker.clockHolder.pm[t === 'ب.ظ' ? 'addClass' : 'removeClass']('active');
		},
		...
		parseTime : function (time, tFormat) {
			...
			// Parse t (am/pm)
			if (is24Hour) t = parseInt(hour) > 11 ? (tLength > 1 ? 'PM' : 'pm') : (tLength > 1 ? 'ق.ظ' : 'am');
